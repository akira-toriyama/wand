...
panel 全体の見た目は `[launcher.row]`、`[launcher.animation]`
(`open` / `close` ∈ `off | fade | pop`)、
`[launcher.decoration]`(`border ∈ off | rainbow`)で調整可。
`[launcher].layout` で panel を `list` / `toolbar` /
...
ジェスチャー / ランチャーの**両方を特定アプリで無効化**したい場合は
...
`[gesture.overlay.trail]` / `[gesture.overlay.badge]` /
`[gesture.overlay]` 配下にある — 全項目は注釈付きで
[`config.toml`](config.toml) に網羅。

`[gesture.recognition].max-segment-ms` で 1 セグメントの制限時間を
...
`[gesture.recognition].cancel-reversals` は緊急脱出 — カーソルを
...
wand --resign           # インストール済 Wand.app を永続自己署名 identity
                          # で再署名 + 再起動(`brew install` / upgrade
                          # 後に TCC 権限が落ちた場合に 1 回だけ)
wand --show-menu --items <PATH> --at <X> <Y> [--selection <TEXT>] \
                 [--title <TEXT>]
                        # 外部トリガー: 任意の [[item]] ファイルで
                          # ランチャーを <X> <Y>(Cocoa 座標、Y-up)
...
互いに矛盾するアクション(例:`wand --reload --quit`)や、対象外の
アクションへ修飾フラグを付けた(例:`--reload --items`)場合は
exit `2`。silent fallback はしない。

daemon は **config.toml を保存時に自動リロード**する(`--reload` は手動
トリガー)。`--reload` / `--status` / `--quit` / `--show-menu` は
クライアントコマンド — daemon が居なければ exit 3 で拒否。
