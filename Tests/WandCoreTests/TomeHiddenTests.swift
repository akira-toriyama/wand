...
/// Session-only context-menu delete (wand#128): `TomeHidden.apply` is
/// the pure per-level filter. Elements whose id is in the hidden set
/// drop; nil-id elements (headers / placeholders) always survive;
/// unknown hidden ids are no-ops.
final class TomeHiddenTests: XCTestCase {

    private func apply(_ ids: [String], _ hidden: Set<String>) -> [String] {
        TomeHidden.apply(ids, id: { $0 }, hidden: hidden)
    }
    ...
    func testDuplicateIdsHideTogether() {
        ...
        // TomeOrderTests.testDuplicateIdsStayStable).
