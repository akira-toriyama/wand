// Session-only context-menu delete (wand#128) — the pure half of the
// feature. The adapter's panel records "the user deleted this row" as
// a node-id set per panel level; this filter drops those rows the next
// time the same level is built. The recursive walk and the empty-folder
// pruning live adapter-side (`PanelTree.applyHidden`) because the tree
// type does; this level filter is the testable core.

public enum TomeHidden {
